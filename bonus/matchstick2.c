...
** Last update Fri Feb 17 16:56:25 2017 ferchaud kevin
*/
...
int	main2(char **map, char **av, char **test, sfRenderWindow *window)
{
  while (1)
    {
      printmap(map);
      my_putchar('\n');
      while ((test = my_turn(map, av[1], av[2], 0, window)) == NULL);
      if (isexit(test) == 1)
	{
	  my_putchar('\n');
	  return (0);
	}
      map = my_turn(map, av[1], av[2], 1, window);
      if (isfinished(map) == 1)
	{
	  printmap(map);
